...
func (q *queueProcessor) scan(ctx context.Context) error {
	...
	if q.capacity() == 0 || q.partitionCapacity() == 0 {
		metrics.IncrQueueScanNoCapacityCounter(ctx, metrics.CounterOpt{PkgName: pkgName, Tags: map[string]any{"queue_shard": q.Shard().Name()}})
		return nil
	}
	...
	if processAccount {
		...
		if len(q.runMode.ExclusiveAccounts) > 0 {
			...
			peeked, err := Duration(ctx, q.Shard().Name(), "account_peek", q.Clock().Now(), func(ctx context.Context) ([]uuid.UUID, error) {
				return q.Shard().AccountPeek(ctx, q.isSequential(), peekUntil, AccountPeekMax)
			})
			if err != nil {
