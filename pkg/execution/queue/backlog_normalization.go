...
func (q *queueProcessor) iterateNormalizationShadowPartition(ctx context.Context, accountID *uuid.UUID, peekLimit int64, until time.Time, bc chan normalizeWorkerChanMsg) error {
	...
	sequential := false
	shadowPartitions, err := q.Shard().PeekShadowPartitions(ctx, accountID, sequential, peekLimit, until)
	if err != nil {
	...
	for _, partition := range shadowPartitions {
		backlogs, err := Duration(ctx, q.Shard().Name(), "normalize_peek", until, func(ctx context.Context) ([]*QueueBacklog, error) {
			return q.Shard().ShadowPartitionPeekNormalizeBacklogs(ctx, partition, NormalizePartitionPeekMax)
		})
		if err != nil {
			return err
		}
		...
		for _, bl := range backlogs {
			// lease the backlog
			_, err := Duration(ctx, q.Shard().Name(), "normalize_lease", q.Clock().Now(), func(ctx context.Context) (any, error) {
				err := q.Shard().LeaseBacklogForNormalization(ctx, bl)
				return nil, err
			})
			if err != nil {
...
func (q *queueProcessor) NormalizeBacklog(ctx context.Context, backlog *QueueBacklog, sp *QueueShadowPartition, latestConstraints PartitionConstraintConfig) error {
	...
	metrics.ActiveBacklogNormalizeCount(ctx, 1, metrics.CounterOpt{PkgName: pkgName, Tags: map[string]any{"queue_shard": q.Shard().Name()}})
	defer metrics.ActiveBacklogNormalizeCount(ctx, -1, metrics.CounterOpt{PkgName: pkgName, Tags: map[string]any{"queue_shard": q.Shard().Name()}})
	...
	go func() {
		for {
			...
			case <-time.Tick(BacklogNormalizeLeaseDuration / 2):
				if err := q.Shard().ExtendBacklogNormalizationLease(ctx, q.Clock().Now(), backlog); err != nil {
					switch err {
	...
	for {
		...
		res, err := q.Shard().BacklogNormalizePeek(ctx, backlog, NormalizeBacklogPeekMax)
		if err != nil {
			if errors.Is(err, context.Canceled) {
				return nil
			}
...
func (q *queueProcessor) NormalizeItem(
	...
	if _, err := q.Shard().EnqueueItem(ctx, item, time.UnixMilli(item.AtMS), EnqueueOpts{
