...
func (q *queueProcessor) runScavenger(ctx context.Context) {
	...
	leaseID, err := q.Shard().ConfigLease(ctx, "scavenger", ConfigLeaseDuration, q.scavengerLease())
	...
	for {
		...
		case <-scavenge.Chan():
			// Scavenge the items
			if q.isScavenger() {
				count, err := q.Shard().Scavenge(ctx, ScavengePeekSize)
				if err != nil {
		...
		case <-tick.Chan():
			...
			if q.scavengerLeaseID == nil {
				...
				// a lease.
				metrics.IncrQueueSequentialLeaseClaimsCounter(ctx, metrics.CounterOpt{PkgName: pkgName, Tags: map[string]any{"queue_shard": q.Shard().Name()}})
			}
